...
public class Exercises {
	 ...
	 * the same order array2List( {"Apple", "Orange", "Banana"} ) ->
	 * ["Apple","Orange", "Banana"] array2List( {"Red", "Orange", "Yellow"} ) ->
	 * ["Red","Orange", "Yellow"] array2List( {"Left", "Right", "Forward", "Back"} )
	 * ->["Left", "Right", "Forward", "Back"]
	 */
	public List<String> array2List(String[] stringArray) {	
		// 1/22/2020 in notebook: for, for-each, addAll. loop methodology

		//Define the empty ArrayList to be filled with the String Array that we are given.
		List<String> array2List = new ArrayList<String>();	
		// Create a for-each loop containing a temporary string and loop 
		// through stringArray assigning each element to temp String fruitColorDirection
		
		for (String fruitColorDirection : stringArray) {
			array2List.add(fruitColorDirection);	
			// adding the temporary string to array2List
		}
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for (String listData : stringArray) {
//			System.out.print(listData);
			if (listData.length() != 4) {
				returnedList.add(listData);
			}
	...
	public List<String> reverseList(List<String> stringList) {
		...
		List<String> reverseList = new ArrayList <String>();
			...
			for (String tempString : stringList) {
				reverseList.add(0, tempString);
				
			}
			return reverseList;
			
//			Stack <String> reverseList = new Stack<String>();
//				List<String> poppedReverseList = new ArrayList <String>();
//				reverseList.addAll(stringList);
//					
//				while (reverseList.size() > 0)
//				{    
//				    poppedReverseList.add(reverseList.pop()); // the next item is removed from the stack
//				    System.out.println(reverseList);    
//				}
//				return poppedReverseList;
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		List<Double> twoDoubles = new ArrayList<Double>();

		for (Integer newDoubles : intArray) {
			twoDoubles.add(newDoubles / 2.0);
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		for (Integer twiceFound : integerList) {
			if (twiceFound == intToFind)
				foundIntTwice++;
		}
		if (foundIntTwice >= 2) {
			return true;
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (Integer whatsFizzBuzz : integerArray) {
			if (whatsFizzBuzz % 3 == 0 && whatsFizzBuzz % 5 == 0) {
				fizzBuzzList.add("FizzBuzz");
			} else if (whatsFizzBuzz % 3 == 0) {
				fizzBuzzList.add("Fizz");
			} else if (whatsFizzBuzz % 5 == 0) {
				fizzBuzzList.add("Buzz");
			} else {
				fizzBuzzList.add(whatsFizzBuzz.toString());
			}
		}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		List<Integer> interLeaveLists = new ArrayList<Integer>(); // Start by creating an output
		
		  int shorterLength = listOne.size();        //Hold the length of the shorter list- start with listOne (assume one condition is true)
		    
		    if (listOne.size() > listTwo.size()) {     //If listTwo is shorter  (i.e. if our assumption was not true)
		        shorterLength = listTwo.size();        //Reset shorterLength to listTwo length
		    }
		    
		    int i = 0;
		    for (i = 0; i < shorterLength; i++) {      // Loop for the length of listOne
		        
		        interLeaveLists.add(listOne.get(i));   // Copy the element at i from listOne to interLeaveLists
		        interLeaveLists.add(listTwo.get(i));   // Copy the element at i from listTwo to interLeaveLists 
		    }
		    
		// Figure out which one was longer
		// Copy the remaining elements to the new list
		    // 1. Have to know where we left off in the longer one
		    // 2. Start after the last one copied   
		    
		    if (listOne.size() > listTwo.size()) {            // If listOne is longer
		         for (int j = i; j < listOne.size(); j++) {   // copy the remaining elements to interLeaveLists
