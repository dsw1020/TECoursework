...
public class AmericanPlayingCard extends PlayingCard{  // Extends a subclass of playing card
	/************************************************************************************************
	 * Constants for values related to American Playing Cards
	 ***********************************************************************************************/	
	private static final int    DEFAULTCARDVALUE = 0;	// "final" indicates constants
	private static final String DEFAULTCOLOR     = "BLACK"; // Constants cannot be changed
	private static final String DEFAULTSUIT      = "Joker";	//static means only one
	private static final int    MAXVALUE         = 13;	//in this case, only one constant
	private static final int    MINVALUE         = 0;	// they are available to all objects created
	 ...
	 **************************************************************************************************/	
	// Since static data can exist without an object - we cant use an object to invoke them
	// We can define an anonymous method to initialize them - method without name or parameters
	//
	...
	public AmericanPlayingCard(int value, String suit) {
		// a sub-class ctro must call a super class ctor as the very first thing it does
	...
	private static void initializeMaps() { // must be a static method since it changes static data (the Maps)
		suitMap.put("SPADES"  , "BLACK"); // Associating the color with the suit in the map
		suitMap.put("CLUBS"   , "BLACK");
		suitMap.put("DIAMONDS", "RED");
		suitMap.put("HEARTS"  , "RED");
		
		valueMap.put(0,"Joker");	// associating the value with the name of the card in a Map
		valueMap.put(1,"Ace");
		valueMap.put(2,"Two");
