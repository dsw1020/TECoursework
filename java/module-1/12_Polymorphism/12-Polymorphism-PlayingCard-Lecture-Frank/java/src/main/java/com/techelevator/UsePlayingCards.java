...
public class UsePlayingCards {
	...
	public static void main(String[] args) {
		...
		anItalianCard1.showCard();	// Ran the ItalianPlayingCard showCard() method
		
		
		System.out.println("------------Super class assigned sub class object---------");
		
		PlayingCard aCard = aUSACard3;	// A sub-class object may be assigned to a superclass object
		aCard.showCard();				// The class of the object in the variable determines which method is run 
										// 		NOT the class of the object used to run the method. 
		
		aCard = aSwissCard4;			// A sub-class object may be assigned to a superclass object
		aCard.showCard();				// The class of the object in the variable determines which method is run 
										// 		NOT the class of the object used to run the method. 
		
		aCard = anItalianCard1;			// A sub-class object may be assigned to a superclass object
		aCard.showCard();				// The class of the object in the variable determines which method is run 
										// 		NOT the class of the object used to run the method. 
		...
		for (int i = 0; i <someCards.length; i++) {
			someCards[i].showCard();		// The method for the class IN the element is run NOT the class of the element
											//  The element is a PlayingCard Class, the object in the element is a subclass
											// 				so the subclass method ran 
		}
