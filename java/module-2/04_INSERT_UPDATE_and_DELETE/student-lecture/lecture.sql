...
insert into countrylanguage                              -- table you're inserting into
      (countrycode, language, isofficial, percentage)    -- columns to recieve the data - order must match the order of columns in the table
values('USA'      , 'Klingon', false     , 24 )           -- values to place in the column
;                                                        -- order of values must match the order of columns specified
...
rollback;               -- until youre sure you make the change
...
Begin transaction;
        ...
        update country
        set capital = (select id from city where name = 'Houston') -- Have the DBMS find the value i want
        where code = 'USA'
    ;
...
Begin transaction;
        ...
        update country
        set capital = (select id from city where name = 'Washington') -- Have the DBMS find the value i want
        , headofstate = 'Vinny and Ricky'
        where code = 'USA'
    ;
