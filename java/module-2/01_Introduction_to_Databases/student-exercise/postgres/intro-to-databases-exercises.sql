...
from country
where governmentform = 'Monarchy' OR governmentform = 'Constitutional Monarchy' 
OR governmentform = 'Monarchy(Emirate)' OR governmentform = 'Monarchy(Sultanate)' OR governmentform = 'Constitutional Monarchy, Federation'
OR governmentform = 'Constitutional Monarchy (Emirate)' OR governmentform = 'Parlamentary Monarchy'
;
-- 13. The name of all cities in the USA with a population between 1 million and 2 million people (6 rows) 
select name, population 
from city
where population <2000000 AND population > 1000000 AND countrycode = 'USA'
;
