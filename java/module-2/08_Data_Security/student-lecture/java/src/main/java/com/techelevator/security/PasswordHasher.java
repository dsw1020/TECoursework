...
public class PasswordHasher {
    ...
    public String computeHash(String clearTextPassword, byte[] salt) {
        Key key = createKey(clearTextPassword, salt);  // generate a raw version of the encrypted value
        byte[] digest = key.getEncoded();			   // generate an encoded version of the encrypted value
    ...
    private Key createKey(String password, byte[] salt) {
        ...
        // PBEKeySPec - encryption technique
        // clearTextPassword converted to a character array
        // salt - random value used to encrypt the password
        // WORK_FACTOR - delay for generating the encrypted value - make the prcess longer to deter hackers
        // KEYLENGTH - length of the encrypted value
