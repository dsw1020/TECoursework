...
export default {
  name: "product-review",
  props: {               // properties (data) being sent this component from outside it
    name: String,        // name-of-property : data-type
    description: String,  // name-of-property : data-type
    searchValue: String   // this will have the value entered in the SearchBar (v-bind in App.vue)  
  },
  data() {
    return {
      showForm: false,
      filter: 0,
      newReview: {},
      /* an object in the reviews array contains:
          
          id:       - unique identifier for the review
          reviewer: - the name of person who submitted revew
          rating:   - 1-5 rating of the review
          title:    - Title of the review
          review:   - the contentof the reviews - the words of the reviewer
      */
      reviews: []
    };
  },
  computed: {
    ...
    filteredReviews(vm) {
      // First, filter reviews based on the star rating filter
      const ratingFilteredReviews = vm.reviews.filter(review => {  // Store the reviews filtered by rating
        return vm.filter === 0 ? true : vm.filter === review.rating;
      });
      // Then, filter the reviews based on the searchValue from the searchBar
      return ratingFilteredReviews.filter((aReview) => {  // Filter the reviews filtered by rating using serachBar value
        return aReview.review.includes(vm.searchValue);   // Does the current review contain the text from searchBar
      });                                                 //       in it's review content
    },
