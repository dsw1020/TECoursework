...
function loadReview() {
  ...
  //  fetch('http://5dd69774ce4c300014403ae7.mockapi.io/productdata')
      fetch('http://localhost:8080/11-Frank-Product-Reviews-MVC-API-Backend/api')
     .then ((response) => {      // when fetch is done - send the data to a function
         return response.json()  // take the response from th fetch and convert the data in teh body to json
        })  //    json() method is an async process that issues a promise
     .then ((theData) => {       //        once data is converted to json
       reviews = theData;        //            assign it to our array
       displayReviews();
     })
     .catch((err) => {   // if there is error in the fetch, just display it
       console.log(err);
      })
     }
