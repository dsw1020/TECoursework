...
public class AccountController {
    ...
    public String index(ModelMap modelHolder) {         // Give me a ModelMap to share with other things in the request
        modelHolder.put("user", auth.getCurrentUser()); // Get the current user and place it in the modelMap with the key "user"

        return "index";                                 // return the name view (.jsp file)
    }
    ...
    public String register(@Valid @ModelAttribute("user") User user    // Server should validate using the "user" in ModelMap and User POJO
        ...
        if (result.hasErrors()) {                                      // If there are any error...
            flash.addFlashAttribute("user", user);                     //                send the data entered from user back to view
            flash.addFlashAttribute(BindingResult.MODEL_KEY_PREFIX + "user", result); // send the validation result back to review
            flash.addFlashAttribute("message", "Please fix the following errors:");   // send a general message to be display concerning errors
